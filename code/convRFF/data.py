...
def random_translation(img, mask, translation_h_w):
    ...
    # Replacing tfa.image.translate with tf.keras.preprocessing.image.apply_affine_transform
    img = tf.keras.preprocessing.image.apply_affine_transform(img.numpy(), tx=dy, ty=dx, fill_mode='nearest')
    mask = tf.keras.preprocessing.image.apply_affine_transform(mask.numpy(), tx=dy, ty=dx, fill_mode='constant')
...
def data_augmentation_func(flip_left_right=True,
    ...
    def data_aug(img, mask):
        ...
        if range_rotate:
            ...
            # Replacing tfa.image.rotate with tf.keras.preprocessing.image.apply_affine_transform
            img = tf.keras.preprocessing.image.apply_affine_transform(img.numpy(), theta=rotation * 180 / np.pi, fill_mode='reflect')
            mask = tf.keras.preprocessing.image.apply_affine_transform(mask.numpy(), theta=rotation * 180 / np.pi, fill_mode='constant')
